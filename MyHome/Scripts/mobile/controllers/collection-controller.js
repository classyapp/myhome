...
classy.controller('CollectionController', function ($scope, $http, $q, AppSettings, ClassyUtilities, Localizer, $routeParams, $location, AuthProvider, $timeout, $route) {
    ...
    AppSettings.then(function (appSettings) {
        ...
        $http.get(appSettings.ApiUrl + '/collection/' + $routeParams.collectionId + '?includeComments=true&includeCommenterProfiles=true&includeListings=true&increaseViewCounter=true&includeProfile=true', config).success(function (data) {
            ...
            promises.push(Localizer.Get('Mobile_CollectionPage_Favorites', appSettings.Culture).then(function (resource) { $scope.Resources.Favorites = resource; }));
            promises.push(Localizer.Get('Mobile_CollectionPage_Comments', appSettings.Culture).then(function (resource) { $scope.Resources.Comments = resource; }));
            promises.push(Localizer.Get('Mobile_CollectionPage_CommentsSectionTitle', appSettings.Culture).then(function (resource) { $scope.Resources.CommentsSectionTitle = resource; }));
            promises.push(Localizer.Get('Mobile_CollectionPage_CommentsFirstTitle', appSettings.Culture).then(function (resource) { $scope.Resources.CommentsFirstTitle = resource; }));
            promises.push(Localizer.Get('Mobile_CollectionPage_NewCommentPlaceholder', appSettings.Culture).then(function (resource) { $scope.Resources.NewCommentPlaceholder = resource; }));
